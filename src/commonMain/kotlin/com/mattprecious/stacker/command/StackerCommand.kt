...
abstract class StackerCommand {
  internal sealed interface State {
     ...
     */
    data class Rendering<R>(val content: @Composable (onResult: (R) -> Unit) -> Unit) : State
  ...
  @Composable
  internal fun Work(onFinish: (Boolean) -> Unit, workState: WorkState = rememberWorkState()) {
...
suspend fun StackerCommand.run(): Boolean {
  ...
  runMosaic { Work(onFinish = { result = it }) }
