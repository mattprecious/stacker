...
internal class PrettyBranch(val branch: TreeNode<Branch>, val pretty: String)
...
private fun TreeNode<Branch>.prettyTree(
  ...
  builder += PrettyBranch(branch = this, pretty = pretty)
}

private fun TreeNode<Branch>.treeWidth(filter: ((TreeNode<Branch>) -> Boolean)): Int {
  return if (!filter(this)) {
    0
  } else {
...
private sealed interface Symbols {
  ...
  fun branch(selected: Boolean, childCount: Int, hasParent: Boolean): String {
    return when {
      hasParent ->
        when (childCount) {
