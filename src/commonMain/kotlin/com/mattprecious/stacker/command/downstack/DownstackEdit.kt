...
private fun Edit(
  ...
  Column(verticalArrangement = spacedBy(1)) {
...
private class EditState(items: ImmutableList<String>) {
    ...
    state.items.forEachIndexed { index, item ->
      val text = buildAnnotatedString {
        promptItem(index == state.selected) {
          val strikethroughIndex =
            if (item.state != EditState.Item.State.Keep) {
