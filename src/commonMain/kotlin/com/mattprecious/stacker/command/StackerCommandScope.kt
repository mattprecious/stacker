...
class StackerCommandScope internal constructor(
	...
	suspend fun <R> render(content: @Composable (onResult: (R) -> Unit) -> Unit): R {
		state.state.value = StackerCommand.State.Rendering(content)
		return snapshotFlow { state.state.value }
			.filterIsInstance<StackerCommand.State.DeliveringRenderResult<R>>()
			.first()
			.result
			.also {
				state.state.value = StackerCommand.State.Working
			}
	}
	...
	suspend fun abort(): Nothing {
		require(state.state.value !is StackerCommand.State.TerminalState)
		state.state.value = StackerCommand.State.Aborted
		awaitCancellation()
	}
}
