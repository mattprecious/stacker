...
class RealConfigManager(db: RepoDatabase, fs: FileSystem, private val stackManager: StackManager) :
  ConfigManager {
  private val repoConfigQueries = db.repoConfigQueries

  private var userConfig: UserConfig by
    jsonFile(
      fs = fs,
      ...
      maximumAllowedPermissions =
        Permissions.Posix(
          setOf(Permissions.Posix.Permission.OwnerRead, Permissions.Posix.Permission.OwnerWrite)
        ),
    ) {
      UserConfig(githubToken = null)
    }
  ...
  override suspend fun initializeRepo(
    ...
    val trailingChangingWithChildren =
      trailingTrunkChanging && currentTrailingTrunkBranch?.children?.isNotEmpty() == true
    val trunkChangingWithChildren =
      trunkChanging &&
        currentTrunkBranch?.children?.any { it != currentTrailingTrunkBranch } == true
    ...
    repoConfigQueries.insert(trunk = trunk, trailingTrunk = trailingTrunk)
    ...
    if (trailingTrunkChanging) {
      trailingTrunk?.let {
        stackManager.trackBranch(branchName = it, parentName = trunk, parentSha = trunkSha)
      }
    }
  }
}
