...
class GitVersionControl(scope: MemScope, private val fs: FileSystem, private val shell: Shell) :
  VersionControl {
  ...
  override fun renameBranch(branchName: String, newName: String) = memScoped {
    checkError(
      ...
      result =
        git_branch_move(allocPointerTo<git_reference>().ptr, getBranch(branchName).ptr, newName, 0),
    )
  }
  ...
  override fun latestCommitInfo(branchName: String): CommitInfo = memScoped {
    ...
    return CommitInfo(title = title, body = body)
  }
  ...
  override fun restack(branchName: String, parentName: String, parentSha: String): Boolean =
    memScoped {
      ...
      val upstreamId =
        withAlloc<git_oid> { checkError("git_oid_fromstr", git_oid_fromstr(it.ptr, parentSha)) }
  ...
  private fun pushBranchesLibGit(branchNames: List<String>): Unit = memScoped {
    val refs =
      withAlloc<git_strarray> {
        it.count = branchNames.size.toULong()
        it.strings =
          branchNames.map { it.asBranchRevSpec() }.map { "+$it:$it" }.toCStringArray(this)
      }

    // TODO: Atomic? I don't think libgit2 supports this.
    val options =
      withAlloc<git_push_options> {
        checkError(
  ...
  private fun MemScope.checkout(branchName: String, treeish: git_object) {
  ...
  private fun MemScope.checkoutTree(treeish: git_object) {
    val options =
      withAlloc<git_checkout_options> {
  ...
  private data class MergeAnalysis(
    ...
    val preferenceNoFastForward =
      mergePreferenceFlags and GIT_MERGE_PREFERENCE_NO_FASTFORWARD.toInt() != 0
  }
  ...
  private fun <T> MemScope.mapBranches(
    ...
    return buildList { forEachBranch(iterator, flags) { add(transform(it)) } }
  }
  ...
  private fun MemScope.performRebase(rebase: git_rebase, branchName: String): Boolean {
    ...
    forEachRebaseOperation(rebase) {
      if (
        !performRebaseOperation(
          rebase = rebase,
          operation = it.pointed,
          committer = signature.pointed!!,
        )
      ) {
  ...
  private fun MemScope.setBranchTarget(branchName: String, target: git_oid) {
  ...
  private inline fun <reified T : CVariable> MemScope.withAlloc(block: (out: T) -> Unit): T {
  ...
  private fun checkError(functionName: String, result: Int) {
...
class LibGit2Error(code: Int, functionName: String, errorMessage: String) :
  RuntimeException(
    message =
      """
