...
@Composable
fun Prompt(message: String, hideInput: Boolean = false, onSubmit: (String) -> Unit) {
  Prompt(
...
@Composable
fun Prompt(message: AnnotatedString, hideInput: Boolean = false, onSubmit: (String) -> Unit) {
...
@Composable
fun YesNoPrompt(message: String, default: Boolean? = null, onSubmit: (Boolean) -> Unit) {
  YesNoPrompt(
    message = remember(message) { message.toAnnotatedString() },
    default = default,
    onSubmit = onSubmit,
...
@Composable
fun YesNoPrompt(message: AnnotatedString, default: Boolean? = null, onSubmit: (Boolean) -> Unit) {
  val printer = LocalPrinter.current
  var input by remember { mutableStateOf("") }

  val prompt =
    remember(message, default) {
      buildAnnotatedString {
        append(message)
        append(" ")
...
class PromptState<T>(
  ...
  @Immutable
  internal inner class Option(val option: T) {
    val display: AnnotatedString
      get() = displayTransform(option)

    val value: AnnotatedString
      get() = valueTransform(option)
  }
  ...
  internal val filteredOptions by derivedStateOf {
    options.map(::Option).filter { filter.isEmpty() || it.value.contains(filter) }
  }
  ...
  internal fun filterAppend(char: Char) {
    ...
    highlighted =
      prioritizedNextHighlight?.firstNotNullOfOrNull { nextPrioritized ->
        filteredOptions
    ...
    highlighted =
      when {
        ...
        lastHighlighted != null -> {
          ...
          filteredOptions.indexOfFirst { optionKey(it.option) == key }.coerceAtLeast(0)
        }
  ...
  Column(
    ...
    state.filteredOptions.forEachIndexed { index, option ->
      val text = buildAnnotatedString {
        promptItem(index == state.highlighted) { append(option.display) }
      }
