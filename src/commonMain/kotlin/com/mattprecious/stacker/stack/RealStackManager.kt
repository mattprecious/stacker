...
class RealStackManager(db: RepoDatabase) : StackManager {
  ...
  override fun untrackBranches(branches: Set<String>) {
    branchQueries.transaction { branches.forEach { untrackBranch(it) } }
  }
  ...
  override fun renameBranch(branch: Branch, newName: String) {
    branchQueries.rename(oldName = branch.name, newName = newName)
  }

  override fun updateParent(branch: String, parent: String) {
    branchQueries.updateParent(branch = branch, parent = parent)
  }

  override fun updateParentSha(branch: Branch, parentSha: String) {
    branchQueries.updateParentSha(branch = branch.name, parentSha = parentSha)
  }

  override fun updatePrNumber(branch: Branch, prNumber: Long) {
    branchQueries.updatePrNumber(branch = branch.name, prNumber = prNumber)
  }
  ...
  private fun getTree(): TreeNode<Branch>? {
    ...
    return if (elements.isEmpty()) {
      ...
      treeOf(elements = elements, keySelector = { it.name }, parentSelector = { it.parent })
    }
  }
}
