...
class UpstackOntoTest {
  ...
  fun untracked() = withTestEnvironment {
    ...
    testCommand({ upstackOnto() }) {
      awaitFrame(
        static =
          "Cannot retarget change-a since it is not tracked. Please track with st branch track.",
        output = "",
      )
  ...
  fun trunk() = withTestEnvironment {
    ...
    testCommand({ upstackOnto() }) {
      awaitFrame(static = "Cannot retarget a trunk branch.", output = "")
  ...
  fun single() = withTestEnvironment {
    ...
    testCommand({ upstackOnto() }) {
      ...
      awaitFrame(static = "Select the parent branch for change-a: main", output = "")
  ...
  fun linearToFork() = withTestEnvironment {
    ...
    testCommand({ upstackOnto() }) {
      ...
      awaitFrame(static = "Select the parent branch for change-b: main", output = "")
  ...
  fun withChildren() = withTestEnvironment {
    ...
    assertThat(gitLog("change-c"))
      .containsExactly("ecaa71f Change C", "23535fb Change B", "94cf6f0 Empty")
  }
}
