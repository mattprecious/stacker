...
class DownstackEditTest {
  ...
  fun errorsWhenOnTrunk() = withTestEnvironment {
    ...
    testCommand({ downstackEdit() }) {
      awaitFrame(static = "Not on a stack.", output = "")
  ...
  fun errorsWhenOnUntrackedBranch() = withTestEnvironment {
    ...
    testCommand({ downstackEdit() }) {
      awaitFrame(
        static =
          "Cannot edit downstack since feature is not tracked. Please track with st branch track.",
        output = "",
      )
  ...
  fun errorsWhenStackHasOneBranch() = withTestEnvironment {
    ...
    testCommand({ downstackEdit() }) {
      awaitFrame(static = "Stack only has one branch.", output = "")
  ...
  fun swappingTwoBranches() = withTestEnvironment {
    ...
    withDatabase {
      assertThat(it.branchQueries.selectAll().executeAsList().map { it.name to it.parent })
        .containsExactlyInAnyOrder("change-a" to "change-b", "change-b" to "main", "main" to null)
    }

    assertThat(gitLog("main")).containsExactly("94cf6f0 Empty")

    assertThat(gitLog("change-b")).containsExactly("23535fb Change B", "94cf6f0 Empty")

    assertThat(gitLog("change-a"))
      .containsExactly("32b0f41 Change A", "23535fb Change B", "94cf6f0 Empty")

    assertThat(gitBranches()).containsExactly("change-a", "* change-b", "main")
  }
  ...
  fun removingBranch() = withTestEnvironment {
    ...
    withDatabase {
      assertThat(it.branchQueries.selectAll().executeAsList().map { it.name to it.parent })
        .containsExactlyInAnyOrder("change-a" to "main", "change-b" to "main", "main" to null)
    }
    ...
    assertThat(gitLog("change-a")).containsExactly("3d9b7d7 Change A", "94cf6f0 Empty")
  ...
  fun removingCurrentBranch() = withTestEnvironment {
    ...
    assertThat(gitLog("change-b"))
      .containsExactly("929371e Change B", "3d9b7d7 Change A", "94cf6f0 Empty")
  ...
  fun untrackingBranch() = withTestEnvironment {
    ...
    withDatabase {
      assertThat(it.branchQueries.selectAll().executeAsList().map { it.name to it.parent })
        .containsExactlyInAnyOrder("change-b" to "main", "main" to null)
    }
  ...
  fun untrackingCurrentBranch() = withTestEnvironment {
    ...
    withDatabase {
      assertThat(it.branchQueries.selectAll().executeAsList().map { it.name to it.parent })
        .containsExactlyInAnyOrder("change-a" to "main", "main" to null)
    }
  ...
  fun deletingBranch() = withTestEnvironment {
    ...
    assertThat(gitBranches()).containsExactly("* change-b", "main")
  }
  ...
  fun deletingCurrentBranch() = withTestEnvironment {
    ...
    assertThat(gitBranches()).containsExactly("* change-a", "main")
  }
  ...
  fun cancelling() = withTestEnvironment {
    ...
    withDatabase {
      assertThat(it.branchQueries.selectAll().executeAsList().map { it.name to it.parent })
        .containsExactlyInAnyOrder("change-b" to "change-a", "change-a" to "main", "main" to null)
    }
