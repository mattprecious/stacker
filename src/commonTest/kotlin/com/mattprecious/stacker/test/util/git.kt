...
fun TestEnvironment.gitAdd(vararg files: Path) {
...
fun TestEnvironment.gitCommit(message: String): Sha {
  // TODO: Escaping.
  environment.exec("git commit --allow-empty -m \"$message\"")
  return gitSha()
}

fun TestEnvironment.gitCheckoutBranch(name: String) {
  environment.exec("git checkout $name")
}

fun TestEnvironment.gitCreateBranch(name: String, startPoint: String = "HEAD") {
  environment.exec("git branch $name $startPoint")
}

fun TestEnvironment.gitCreateAndCheckoutBranch(name: String) {
...
fun TestEnvironment.gitSetDefaultBranch(name: String) {
