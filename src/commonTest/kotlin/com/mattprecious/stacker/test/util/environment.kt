...
internal fun withTestEnvironment(validate: suspend TestEnvironment.() -> Unit) {
...
class TestEnvironment(
  ...
  suspend fun withDatabase(path: Path = defaultDbPath, block: (RepoDatabase) -> Unit) {
  ...
  suspend fun testCommand(
    ...
    withStacker { it.commandBuilder().test(validate) }
  }

  private suspend fun withStacker(block: suspend (StackerDeps) -> Unit) {
    withStacker(fileSystem = fileSystem, remoteOverride = remote, block = block)
  }
}
