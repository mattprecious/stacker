...
suspend fun StackerCommand.test(validate: suspend CommandTestScope.() -> Unit) {
  runMosaicTest(MosaicSnapshots) { CommandTestScope(command = this@test, mosaic = this).validate() }
}

class CommandTestScope
internal constructor(command: StackerCommand, private val mosaic: TestMosaic<Mosaic>) {
  ...
  suspend fun awaitFrame(output: String, static: String = "") {
  ...
  suspend fun awaitResult(): Boolean {
    ...
    withTimeout(1.seconds) {
      snapshotFlow { state.state }.filter { it is State.TerminalState }.first()
    }
